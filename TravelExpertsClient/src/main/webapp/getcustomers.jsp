...
<html>
  <head>
    ...
    <script>
        ...
        $.getJSON(
          ...
          function (data) {
            ...
            $.each(data, function (index, customer) {
              var customerName =
                customer.custLastName + ", " + customer.custFirstName;
              mySelect.append(
                $("<option>").text(customerName).val(customer.customerId)
              );
            });
          }
        ).fail(function () {
      ...
      function loadSelectAgents() {
        $.getJSON(
          "http://localhost:8080/TravelExpertsRESTJPA-1.0-SNAPSHOT/api/agent/getallagents",
          function (data) {
      ...
      function getCustomer(customerId) {
        ...
        $.getJSON(
          "http://localhost:8080/TravelExpertsRESTJPA-1.0-SNAPSHOT/api/customer/getcustomer/" +
            customerId,
          function (data) {
      ...
      function updateCustomer() {
        ...
        $.ajax({
          url:
            "http://localhost:8080/TravelExpertsRESTJPA-1.0-SNAPSHOT/api/customer/putcustomer/" +
            $("#customerselect").val(),
          type: "PUT",
          contentType: "application/json",
          data: JSON.stringify(customerData),
      ...
      function deleteCustomer() {
        ...
        $.ajax({
          url:
            "http://localhost:8080/TravelExpertsRESTJPA-1.0-SNAPSHOT/api/customer/deletecustomer/" +
            customerId,
          type: "DELETE",
          success: function (response) {
      ...
      function validateForm() {
        return (
          $("#custFirstName").val().trim() && $("#custLastName").val().trim()
        );
      }
  ...
  <body>
    <form>
      ...
      <select id="agentId">
        <option value="">Select an agent</option></select
      ><br />
